...
usage() {
  cat <<'USAGE'
Usage: push-gpu-experiment-runner-image.sh [--image-ref <image>] [--username <user>]

Build the GPU experiment runner image locally and push it to GHCR.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      ;;
    *)
      printf '[push-gpu-experiment-runner-image] unknown argument: %s\n' "$1" >&2
      usage >&2
      exit 1
      ;;
  esac
...
if [[ -z "$REGISTRY_TOKEN" ]]; then
  printf '[push-gpu-experiment-runner-image] GHCR_TOKEN is required\n' >&2
  exit 1
fi

printf '[push-gpu-experiment-runner-image] logging into %s as %s\n' "$REGISTRY_HOST" "$REGISTRY_USERNAME"
printf '%s' "$REGISTRY_TOKEN" | docker login "$REGISTRY_HOST" -u "$REGISTRY_USERNAME" --password-stdin >/dev/null

printf '[push-gpu-experiment-runner-image] building %s\n' "$IMAGE_REF"
docker build -t "$IMAGE_REF" "$ROOT_DIR/services/runner"

printf '[push-gpu-experiment-runner-image] pushing %s\n' "$IMAGE_REF"
docker push "$IMAGE_REF"

printf '[push-gpu-experiment-runner-image] done\n'
